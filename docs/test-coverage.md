...
| Benchmarking     | planned  | recommend: micro-benchmarks listed above + macro-benchmark inside HelixCode profile-run                                                      | Macro tier lives outside Concurrency (CONST-051(B)). |
...
| HelixQA          | planned  | recommend: register Concurrency as a target in HelixQA's autonomous QA bank                                                                  | HelixQA submodule (`HelixDevelopment/HelixQA`) is incorporated at HelixCode root per CONST-050; Concurrency enrolment is a HelixCode-meta-repo task, not a Concurrency-internal task. |
